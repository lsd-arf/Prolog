...
fib(2, _, CurX2, CurX2) :- !.
fib(N, CurX1, CurX2, X) :-
  CurX3 is CurX1 + CurX2,
  N1 is N - 1,
  fib(N1, CurX2, CurX3, X).
...
sumOfDigits(Num, Sum) :-
  Mod is Num mod 10,
  Num1 is Num div 10,
  sumOfDigits(Num1, Sum1),
  Sum is Sum1 + Mod.

% Задание 8
sumOfDigits(0, CurSum, CurSum) :- !.
sumOfDigits(Num, CurSum, Sum) :-
  Mod is Num mod 10,
  Num1 is Num div 10,
  CurSum1 is CurSum + Mod,
  sumOfDigits(Num1, CurSum1, Sum).
...
minDigit(Num, CurMin, Min) :-
  CurMin1 is Num mod 10,
  Num1 is Num div 10,
  (CurMin1 < CurMin ->
  CurMin2 is CurMin1;
  CurMin2 is CurMin),
  minDigit(Num1, CurMin2, Min).
...
% рекурсия вниз
countDigits(Num, CurCount, Count) :-
  Num < 3,
  Count is CurCount + 1, !.
countDigits(Num, CurCount, Count) :-
  Num >= 3,
...
countDigits(Num, CurCount, Count) :-
  ...
  countDigits(Num1, CurCount1, Count).
...
nodNums(_, _, 1, 1) :- !.
nodNums(Num1, Num2, CurNod, Nod) :-
  Num1 mod CurNod =:= 0,
  Num2 mod CurNod =:= 0,
  Nod is CurNod, !.
nodNums(Num1, Num2, CurNod, Nod) :-
  CurNod1 is CurNod - 1,
  nodNums(Num1, Num2, CurNod1, Nod).
nodNumsN(Num1, Num2, Nod) :-
  (Num1 > Num2 ->
  nodNums(Num1, Num2, Num2, Nod);
  nodNums(Num1, Num2, Num1, Nod)).

% Проверка на простоту
simpleNum(_, 1, 1) :- !.
simpleNum(Num, CurDel, Del) :-
  not(Num mod CurDel =:= 0),
  CurDel1 is CurDel - 1,
  simpleNum(Num, CurDel1, Del).
simpleNumN(1) :- !.
simpleNumN(Num) :-
  CurDel is Num - 1,
  simpleNum(Num, CurDel, _).

% Количество делителей
countDels(_, 0, CurCount, CurCount) :- !.
countDels(Num, CurDel, CurCount, Count) :-
  (Num mod CurDel =:= 0 ->
  CurCount1 is CurCount + 1; 
  CurCount1 is CurCount),
  CurDel1 is CurDel - 1,
  countDels(Num, CurDel1, CurCount1, Count).
...
sumOfDels(Num, CurDel, Sum) :-
  CurDel1 is CurDel - 1,
  sumOfDels(Num, CurDel1, Sum1),
  ((Num mod CurDel =:= 0,
  not(simpleNumN(CurDel))) ->
  Sum is Sum1 + CurDel;
  Sum is Sum1).
...
sumOfDels(Num, CurDel, CurSum, Sum) :-
  CurDel1 is CurDel - 1,
  ((Num mod CurDel =:= 0,
  not(simpleNumN(CurDel))) ->
  CurSum1 is CurSum + CurDel;
  CurSum1 is CurSum),
  sumOfDels(Num, CurDel1, CurSum1, Sum).
...
sumOfSimpleDigits(Num, CurSum, Sum) :-
  Mod is Num mod 10,
  Num1 is Num div 10,
  (simpleNumN(Mod) ->
  CurSum1 is CurSum + Mod;
  CurSum1 is CurSum),
  sumOfSimpleDigits(Num1, CurSum1, Sum).
sumOfSimpleDigitsN(Num, Sum) :- sumOfDigits(Num, 0, Sum).

% блок: взаимно простые числа
coprimeNums(Num1, Num2) :-
  nodNumsN(Num1, Num2, Nod),
  Nod = 1.
